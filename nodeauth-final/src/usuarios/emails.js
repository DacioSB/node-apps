...
class EmailNovaSenha extends Email {
  constructor (usuario, token) {
    super()
    this.from = '"Blog do Código" <noreply@blogdocodigo.com.br>'
    this.to = usuario.email
    this.subject = 'Redefinição de Senha'
    this.text = `Olá! Voce pediu para redefinir sua senha ${token}`
    this.html = `<h1>Olá!</h1> Voce pediu para redefinir sua senha. Acesse: <a href="${token}">${token}</a>`
  }
}

module.exports = { EmailVerificacao, EmailNovaSenha }
